...
Item {
	...
	Timer {
		id: idRootTimerForImmobilizerOnOff
		interval: 500
		repeat: false
		...
		onTriggered: {
			if (running === true) {
				console.log ("idRootTimerForImmobilizerOnOff triggered, running")
				bImmobilizerOnOffTriggerSignal = true;
			}
			else {
				console.log ("idRootTimerForImmobilizerOnOff triggered, not running")
...
//					&& (dtcSource !== "03") && (dtcID !== "00FFFF")
	...
	onMachineBrandingChanged:
	{
		...
		if(     (canError18FFD747 === false)
		  &&    (   dataEngine.cfg_MachineBranding.value === 0 // No branding
				||  dataEngine.cfg_MachineBranding.value === 1 // Terex
				||  dataEngine.cfg_MachineBranding.value === 2 // Takeuchi
				||  dataEngine.cfg_MachineBranding.value === 3 // Schaeff
				||  dataEngine.cfg_MachineBranding.value === 5))// OEM4
		{
			var languages = [
				"English",
				"Francais",
		...
		{
			// (canError18FFD747 === true) || (dataEngine.yanmarVersionBoot.value != 0)
...
//			console.log("bImmobilizerOnOffTriggerSignal = " + bImmobilizerOnOffTriggerSignal);
			   ...
			   )
			{
				...
				if(pg_pinCode.checkTypedPin === true)
				{
					...
					console.log("owner pin: " + pg_pinCode.pinToVerify)
			...
			else if((backEnd.state === MyAppState.NEW_TRANS_PIN))
			{
				...
				if(pg_pinCode.checkTypedPin === true)
				{
					...
					console.log("new trans pin input: " + pg_pinCode.pinToVerify)
			...
			else if((backEnd.state === MyAppState.NEW_DRIVER_PIN))
			{
				...
				if(pg_pinCode.checkTypedPin === true)
				{
					...
					u8ImmoByte4 = Byte4 = parseInt(enteredOwnerPin.slice(2,4), 16)
					u8ImmoByte5 = Byte5 = parseInt(enteredOwnerPin.slice(0,2), 16)
					console.log("new driver pin input: " + pg_pinCode.pinToVerify)
					//pg_pinCode.acceptNewPin()
				}
			}
			else if((backEnd.state === MyAppState.NEW_OWNER_PIN))
			{
				...
				if(pg_pinCode.checkTypedPin === true)
				{
					...
					u8ImmoByte4 = Byte4 = parseInt(enteredOwnerPin.slice(2,4), 16)
					u8ImmoByte5 = Byte5 = parseInt(enteredOwnerPin.slice(0,2), 16)
					console.log("new owner pin: " + pg_pinCode.pinToVerify)
					//pg_pinCode.acceptNewPin()
				}
			}
			else if((backEnd.state === MyAppState.IMMOBILIZER))
			{
				u8ImmoByte0 = Byte0 = mcuScreenCTRL; // use the MCU-value
				if(pg_pinCode.checkTypedPin === true)
				{
					pinCodeHex = pg_pinCode.pinToVerify.toString(16).toUpperCase().padStart(4, '0'); // convert uint16 to string hexadecimal
					u8ImmoByte4 = Byte4 = parseInt(pinCodeHex.slice(2,4), 16)
					u8ImmoByte5 = Byte5 = parseInt(pinCodeHex.slice(0,2), 16)
					console.log("entered pin: " + pg_pinCode.pinToVerify)
				}
			}
			else {
...
//				u8ImmoByte0 = 255;
			...
			_CanHandler.sendCAN(419398696, Byte0, Byte1, Byte2, Byte3, Byte4, Byte5, Byte6, Byte7)
//			_CanHandler.sendCAN(419398696, u8ImmoByte0, u8ImmoByte1, u8ImmoByte2, u8ImmoByte3, u8ImmoByte4, u8ImmoByte5, u8ImmoByte6, u8ImmoByte7)
	...
	Timer {
		...
		onTriggered: {
			...
			var Byte6 = 255 // unused
			var Byte7 = 255 // unused
			_CanHandler.sendCAN(419358248, Byte0, Byte1, Byte2, Byte3, Byte4, Byte5, Byte6, Byte7)
			...
			_CanHandler.sendCAN(419421992, Byte0, Byte1, Byte2, Byte3, Byte4, Byte5, Byte6, Byte7)


			// 0x18FFD028 - Machine settings 1
			Byte0 = (dm.indexPTO > 0) ? dm.getPtoSettings(dm.indexPTO - 1).pto1_left * 10 : 100
			Byte1 = (dm.indexPTO > 0) ? dm.getPtoSettings(dm.indexPTO - 1).pto2_left * 10 : 100
			Byte2 = 255
			Byte3 = 255
			Byte4 = 255
			...
			_CanHandler.sendCAN(419418152, Byte0, Byte1, Byte2, Byte3, Byte4, Byte5, Byte6, Byte7)
			...
			Byte6 = 255
			Byte7 = 255
			_CanHandler.sendCAN(419418664, Byte0, Byte1, Byte2, Byte3, Byte4, Byte5, Byte6, Byte7)
...
//            if(targetARM === false && readyForInit === true)
//            {
//                // In screen emulation, start with immobilizer
...
//            }
			...
			if(dataEngine.connectionState == 1 && readyForInit === true)
			{
				...
				if(checkMachineType === true) // check once
				{
					...
					isV7 = dataEngine.cfg_EngineEmissionStandard.value === 1 ? false : true
					//console.log("dataEngine.cfg_EngineEmissionStandard.value: " + dataEngine.cfg_EngineEmissionStandard.value)
...
*/
						console.log("No CAN error for immobilizer")
...
//            console.log("btnMenuFiltrRegenVisible: " + btnMenuFiltrRegenVisible)
			...
			if(isV7 === false)
			{
				...
				{
					buzz7 = true  /// hier supress falsher
				}
				else
				{
					buzz7 = false
			...
			if(   ((mcuScreenCTRL === 2) || (mcuScreenCTRL === 1)) // if MCU want the display to ask for a generic PIN code (2) or the owner PIN code (1)
...
//                                                 + mcuRtcHour + ":" + mcuRtcMinutes + ":" + mcuRtcSeconds)
			...
			{
				RemainingTime.rtMCU = dataEngine.remainingBatteryDisChargingTime.value/3600.0 // remaining time from MCU, also helps compute remaining soc
			}
	...
	Timer {
		...
		onTriggered: {
			...
			if(targetARM){
				...
				// 18FEF147 1s
				canError18FEF147 = canErrorDict["419361095"]
				...
				// 18FFC347 100ms
				canError18FFC347 = canErrorDict["419414855"]
				...
				// 18FF2B03 100ms
				canError18FF2B03 = canErrorDict["419375875"]

				// CFF7147 10ms
				canErrorCFF7147 = canErrorDict["218067271"]

				// 18F11447 100ms
				canError18F11447 = canErrorDict["217126023"] // rename to canError18F11487; 217126023 = CF11487
				...
				// 18FEF700 100ms
				canError18FEF700 = canErrorDict["419362560"]

				// 18FF1100 100ms
				canError18FF1100 = canErrorDict["419369216"]

				// CF00400 100ms
				canErrorCF00400 = canErrorDict["217056256"]

				// 18FEF200 100ms
				canError18FEF200 = canErrorDict["419361280"]

				// 18FF2403 100ms
				canError18FF2403 = canErrorDict["419374083"]

				// CF00300 100ms
				canErrorCF003FE = canErrorDict["217056000"] // rename to canErrorCF00300
				...
				// 18FEDB00 100ms
				canError18FEDB00 = canErrorDict["419355392"]

				// 18FF2703 100ms
				canError18FF2703 = canErrorDict["419374851"]
				...
				canErrorAny = (   canError18FEF147 || canError18FFC047 || canError18FFCE47
							   || canError18FEE747 || canError18FFC347 || canError18FFC147
							   || canError18FFD747 || canError18FFAE47 || canError18FFAF47
							   || canError18FED931 || canError18FF2B03 || canErrorCFF7147
							   || canError18FEDA03 || canError18FEDA47 || canError18FEF700
							   || canError18FF1100 || canErrorCF00400 || canError18FEF200
							   || canError18FF2403 || canErrorCF003FE || canError19F0102B
