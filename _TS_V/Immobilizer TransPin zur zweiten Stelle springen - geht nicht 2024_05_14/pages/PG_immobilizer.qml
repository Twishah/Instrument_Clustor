...
//	BUG: wenn einmailg WFS entspeert, dann wird der Wert ScreenCtrl == 1 oder == 2 nicht mehr richtig akzeptiert
	...
	property string currentInfoMsg: qsTrId("LBL_0349") + translator.emptyString
...
//			console.log("dataEngine.screenCtrl.value " + dataEngine.screenCtrl.value)
					...
					&& (dataEngine.screenCtrl.value === 3)) // 3 = wrong pin
			{
				...
				if((dataEngine.immobTransPinLeftTries.value === 0))
				{
					...
					&& (dataEngine.screenCtrl.value === 4)) // 4 = delay input
			{
				...
				currentInfoMsg = "locked for 15min"
